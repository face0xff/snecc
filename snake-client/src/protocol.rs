...
fn send_data(stream: &mut TcpStream, msg: Msg, msg_data: &[u8]) -> () {
    ...
    match stream.write(
        &[
            &[msg_to_id(msg), (n % 256) as u8, (n / 256) as u8],
            msg_data,
        ]
        .concat(),
    ) {
...
pub fn get_player_id(stream: &mut TcpStream) -> u8 {
    ...
    if msg_id != Msg::PlayerId || msg_len != 1 {
        panic!(
            "get_player_id: malformed message; id:{}; len:{}",
            msg_to_id(msg_id),
            msg_len
        );
    }
...
pub fn get_game_params(stream: &mut TcpStream, game: &mut Game, client_player_id: u8) {
    ...
    if msg_id != Msg::GameParams {
        panic!(
            "get_game_params: malformed message; id:{}; len:{}",
            msg_to_id(msg_id),
            msg_len
        );
    }
    ...
    for i in 0..(n_players as u32) {
        ...
        if player_id == client_player_id {
            game.set_border_color([
                player_red as f32 / 256.0,
                player_green as f32 / 256.0,
                player_blue as f32 / 256.0,
                1.0,
            ]);
        }
    }
...
pub fn deconstruct_frame(msg: Msg, buf: &[u8], game: &mut Game) -> Vec<(u8, bool)> {
    if msg != Msg::Frame {
        panic!(
            "deconstruct_frame: expected id {}, got {}",
            msg_to_id(Msg::Frame),
            msg_to_id(msg)
        );
    }
    ...
    for _i in 0..food_number {
        ...
        game.add_food(
            food_x as i16,
            food_y as i16,
            Food::int_to_food_type(food_type),
        );
    }
